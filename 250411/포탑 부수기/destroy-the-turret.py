...
def find_weak(t,board, towers):
    ...
    return sx,sy,towers,board

#1.2. 강한 공격자 선정
def find_strong(sx, sy,board, towers):
    N = len(board)
    M = len(board[0])
    # 1. 공격력이 가장 낮은 포탑 좌표 리스트를 찾고
...
for i in range(N):
    ...
    for j in range(M):
        towers[(i, j)] = [row[j], 0, False]  # 공격력, 공격시각, 피해여부
    board.append(row)
...
=> 만약 공격력이 가장 낮은 포탑이 2개이상 이면, 
=> => 만약 그러한 포탑이 2개 이상이라면, 각 포탑 위치가장 최근에 공격한 포탑이 가장 약함
=> => (모든 포탑은 시점 0에 모두 공격한 경험이 있다고 가정)의 행과 열의 합이 가장 큰 포탑이 약함
